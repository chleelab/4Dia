		...
		for (k = 1; k < fr+1; k++) {
			for (p = 1; p < 3; p++){
				if (p==1){
					...
					aa=floor(sl/2);
			 		run("Make Substack...", "channels="+ChToUse+" slices=1-"+aa+" frames="+k);
					run("Reverse");
				}
				if (p==2){
					selectWindow(filenames[i]);
			 	...
			 	selectWindow("AlignTemp.tif");
				close(); 

				// align the 1st  channels using vectors gotten from 1st channel.
			 	selectWindow(filenames[i]);
			 	if (p==1){
				...
				if (p==2){
				 ...
				 // align the 2nd  channels using vectors gotten from 1st channel.
			 	selectWindow(filenames[i]);
			 	if (p==1){
			...
			// align the 3rd  channels using vectors gotten from 1st channel.
			 
			 if (p==1){
			 	selectWindow(filenames[i]);
			 		run("Make Substack...", "channels=3"+" slices=1-"+aa+" frames="+k);
					run("Reverse");
				}
			if (p==2){
				selectWindow(filenames[i]);
			...
			if (p==2){
			 ...
			 run("Concatenate...", "  title=[" + title + "-7.tif] image1=[" + title + "-1.tif] image2=[" + title + "-4.tif] image3=[-- None --]" );
			 run("Concatenate...", "  title=[" + title + "-8.tif] image1=[" + title + "-2.tif] image2=[" + title + "-5.tif] image3=[-- None --]" );
			 run("Concatenate...", "  title=[" + title + "-9.tif] image1=[" + title + "-3.tif] image2=[" + title + "-6.tif] image3=[-- None --]" );
			 	
			 // Merge into one multi-channel z-stack
			 run("Merge Channels...", "c7=["+title+"-7.tif] c6=["+title+"-8.tif] c4=["+title+"-9.tif] create");
